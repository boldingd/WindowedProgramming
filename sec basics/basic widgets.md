...
`BasicWidget`s frequently have the ability to contain other <BasicWidget</code>s. This allows us to put `BasicWidget`s into other `BasicWidget`s - or more usefully, other \*classes that inherit from\* `BasicWidget` into other classes that inherit from `BasicWidget`/
